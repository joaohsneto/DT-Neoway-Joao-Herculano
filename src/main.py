...
if __name__ == "__main__":
    ...
    regex = r"""
        |(Dr.\s)|(Mr.\s)|(Mrs.\s)|(Ms.\s)|(Jr.\s)|(Sr.\s)|(Miss)|
        |(I\s)|(II)|(III)|(IV)|(\sV\s)|(MD)|(DDS)|(PhD)|(DVM)|
    """
...
# coletando as informações dos estudantes e inserindo no banco de dados
    for path in cpf_paths:
        ...
        student_name = scrape_student_name(students_html)
        clear_name = re.sub(regex, "", student_name).strip()
        student_score = scrape_student_score(students_html).strip()
        insert_data(student_cpf, clear_name.upper(), float(student_score))
    close_connection()
    print("Done!")
